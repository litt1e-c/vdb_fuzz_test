"""
EDC "HELLFIRE" Runner v3: Extreme Scale Vector DB Verification
Scale: N=200,000 | DIM=768

Changelog v3:
1. Qdrant: Implemented Bulk Upload Best Practices.
   - Disable indexing during ingest.
   - Use `client.upload_points` for robust batching.
   - Re-enable indexing at Flush phase and wait for 'GREEN' status.
2. Milvus: Logic expression syntax fixed (parentheses).
3. General: Full code provided, no snippets.
"""
...
class MilvusAdapter(BaseAdapter):
    ...
    def create_schema(self, name):
        ...
        # Strong consistency is critical for logic tests
    ...
    def search(self, vec, limit, offset=0, expr=None):
        # Dynamic ef calculation for deep paging
        required_k = limit + offset
        dynamic_ef = max(128, required_k + 200)
        ...
        if expr:
            if "tag ==" in expr:
                ...
                # Explicit parentheses for logic safety
...
class QdrantAdapter(BaseAdapter):
    def __init__(self):
        ...
        self.client = QdrantClient("http://localhost:6333", timeout=600) if HAVE_QDRANT else None
        self.col = None
    ...
    def create_schema(self, name):
        ...
        if not self.client.collection_exists(name):
            self.client.create_collection(
                ...
                # IMPORTANT: Disable indexing during ingest
        ...
        for i, v, p in zip(ids, vecs, payloads):
            ...
            for k, val in p.items():
                if isinstance(val, (np.integer, np.floating)):
                    clean_payload[k] = val.item()
                elif isinstance(val, list):
                    clean_payload[k] = [x.item() if hasattr(x, 'item') else x for x in val]
                else:
                    clean_payload[k] = val
            
            points.append(Qmodels.PointStruct(id=int(i), vector=v.tolist(), payload=clean_payload))

        # Use upload_points (Bulk API) instead of upsert
        self.client.upload_points(
            collection_name=self.col,
            points=points,
            batch_size=1024, # Internal batching within the upload
            parallel=4,      # Parallel uploads
            wait=True        # Wait for acknowledgment
        )

    def upsert_single(self, id_val, vec, payload):
        # Single upsert can fallback to standard upsert or insert_batch logic
        self.client.upsert(
            collection_name=self.col,
    ...
    def flush(self):
        print("    [Qdrant] Re-enabling indexing and optimizing...", end="", flush=True)
        # Re-enable indexing
...
def test_deep_pagination(db, q_probe):
    # Offset 8000 safe for most defaults
    OFFSET = 8000
    LIMIT = 50
...
def main():
    ...
    print(f"{'='*60}")
    print(f"🔥 HELLFIRE RUNNER v3: Scale N={N_SCALE}, DIM={DIM}")
