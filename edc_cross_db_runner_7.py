"""
EDC "CHAOS" RUNNER v5: Self-Healing Robustness Verification
Scale: N=200,000 | DIM=768
Fixes:
1. Weaviate: Added Auto-Healing mechanism. If a batch fails (e.g., due to NaN injection), 
   the client instance is destroyed and recreated to clear the poisoned buffer.
   This prevents the 'Out of range float' error from leaking into subsequent tests.
2. Logging: Refined consistency reporting to distinguish 'Eventual Consistency' from 'Data Loss'.
"""
...
class WeaviateAdapter(BaseAdapter):
    ...
    def _reconnect(self):
        """Auto-healing: Recreate client to clear poisoned batch queues."""
        self.client = weaviate.Client("http://localhost:8080", timeout_config=(5, 300))

    def connect(self):
        self._reconnect()

    def ok(self): 
        if not self.client: self._reconnect()
        return HAVE_WEAVIATE and self.client and self.client.is_ready()
    ...
    def setup(self, name):
        self._reconnect() # Start with a fresh client for every test
    ...
    def insert_batch(self, ids, vecs, payloads):
        ...
        try:
            ...
            # [Fix] If batch fails (e.g. NaN), we MUST reconnect to clear the buffer
            self._reconnect()
            raise e
    ...
    def upsert_single(self, id_val, vec, payload):
        ...
        if self.client.data_object.exists(uid, class_name=self.cls):
            self.client.data_object.replace(props, self.cls, uuid=uid, vector=vec.tolist())
        else:
            self.client.data_object.create(props, self.cls, uuid=uid, vector=vec.tolist())
...
def test_dirty_data(adapter):
    ...
    try:
        ...
        # Force a tiny operation to trigger potential batch errors in Weaviate
...
def test_atomic_transfer(adapter):
    ...
    if stats["inconsistent"] > 0:
        # Expected behavior for Eventual Consistency
        return ChaosResult("AtomicTransfer", False, f"Consistency Rate: {consistency_rate:.2%} (Eventual Consistency Detected)")
...
def main():
    ...
    print(f"{'='*60}")
    print(f"🧨 EDC CHAOS RUNNER v5 (Self-Healing)")
