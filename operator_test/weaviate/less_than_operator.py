...
PORT = 8080
GRPC_PORT = 50051
COLLECTION_PREFIX = "LessThanOperatorValidation"


def deterministic_uuid(label):
    return str(uuid.uuid5(uuid.NAMESPACE_DNS, f"weaviate-less-than-operator-{label}"))
...
def main():
    ...
    try:
        ...
        checks.append(print_check(
            "int_less_than_filter_only",
            "LessThan is a documented comparison operator for typed property values.",
            "Current oracle maps INT less_than to strict pandas < on present values.",
            ["a", "b"],
            fetch_tags(collection, Filter.by_property("intFilterOnly").less_than(11)),
        ))
        ...
        try:
            range_only_result = fetch_tags(
                collection,
                Filter.by_property("intRangeOnlyObservation").less_than(11),
            )
            observations.append(("range_only_query_result", range_only_result))
            print_observation(
                "int_less_than_range_only_observation",
            ...
            print_observation(
                "int_less_than_range_only_observation",
                "The local client/server combination did not preserve or use the requested range-only property configuration; this keeps range-only less_than outside the validated oracle subset.",
                {
                    "property_config": str(range_only_cfg),
                    "query_error": str(exc),
                },
        ...
        checks.append(print_check(
            "number_less_than_regular_value",
            "LessThan is documented for numeric values.",
            "Current oracle maps NUMBER less_than to strict float < for regular finite values.",
            ["a", "b", "c"],
            fetch_tags(collection, Filter.by_property("numberVal").less_than(0.5)),
        ))

        checks.append(print_check(
            "date_less_than_utc_normalized",
            ...
            ["a", "b", "c"],
            fetch_tags(collection, Filter.by_property("dateVal").less_than("2024-03-01T00:00:00Z")),
        ))

        checks.append(print_check(
            "nullable_int_less_than_excludes_null_missing",
            "The docs define comparison filtering but do not document a less_than-specific null or missing truth table.",
            "Current oracle treats null or omitted scalar values as non-matches for strict less_than.",
            ["a"],
            fetch_tags(collection, Filter.by_property("nullableInt").less_than(5)),
        ))

        signed_zero_mismatches = []
        for name, expected, actual in [
            (
                "number_less_than_positive_zero_signed_zero_probe",
                ["a"],
                fetch_tags(collection, Filter.by_property("zeroNumberVal").less_than(0.0)),
            ),
            (
                "number_less_than_negative_zero_signed_zero_probe",
                ["a"],
                fetch_tags(collection, Filter.by_property("zeroNumberVal").less_than(-0.0)),
            ),
        ]:
        ...
        if all(checks):
            print("Summary: all less_than operator checks passed on the local Weaviate service.")
            if observations:
                print("Summary note: range-only comparison indexing remained observation-only outside the current validated subset.")
            if signed_zero_mismatches:
                print("Summary note: signed-zero NUMBER less_than mismatches were observed outside the current validated subset.")
            return 0

        print("Summary: one or more less_than operator checks failed.")
        return 1
    finally:
        try:
