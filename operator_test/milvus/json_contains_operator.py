...
COLLECTION_NAME = "json_contains_operator_validation"
...
def main():
    ...
    try:
        ...
        tests = [
            ("scalar_member_contains", 'json_contains(meta["tags"], "sale")', [1]),
            ("numeric_member_contains", 'json_contains(meta["nums"], 2)', [1, 2]),
            ("subarray_contains", 'json_contains(meta["pairs"], [1,2])', [1, 2]),
            ("missing_key_filtered", 'json_contains(meta["missing"], 1)', []),
            ("non_array_filtered", 'json_contains(meta["scalar_tag"], "sale")', []),
            ("raw_not_observed", 'not (json_contains(meta["tags"], "sale"))', [2, 3, 4, 5, 8]),
            (
                "guarded_not_observed",
                'not ((meta is not null and json_contains(meta["tags"], "sale")))',
                [2, 3, 4, 5, 6, 7, 8],
            ),
        ]

        print("--- JSON_CONTAINS operator validation ---")
