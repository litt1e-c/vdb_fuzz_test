...
COLLECTION_NAME = "multiply_operator_validation"
INT64_MAX = 2**63 - 1
INT64_MIN = -(2**63)
...
def main():
    ...
    try:
        ...
        print("--- MULTIPLY operator validation ---")
        safe_tests = [
            ("safe_int_eq", '(c_int is not null and (c_int * 2 == 200))', [2]),
            ("safe_float_gt", '(c_float is not null and (c_float * 2 > 2.5))', [1, 2]),
            ("safe_double_le", '(c_double is not null and (c_double * 2 <= 2.6))', [1]),
        ]
        overflow_probes = [
            ("overflow_false_positive_upper", 'id in [1, 2] and (c_int * 2 < 0)', []),
            ("overflow_false_positive_lower", 'id in [2, 3] and (c_int * 2 > 0)', [2]),
            ("overflow_false_negative_lower", 'id in [2, 3] and (c_int * 2 < 0)', [3]),
        ]
