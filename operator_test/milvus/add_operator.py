...
COLLECTION_NAME = "add_operator_validation"
INT64_MAX = 2**63 - 1
...
def main():
    ...
    try:
        ...
        print("--- ADD operator validation ---")
        safe_tests = [
            ("safe_int_eq", '(c_int is not null and (c_int + 33 == 133))', [2]),
            ("safe_float_gt", '(c_float is not null and (c_float + 2 > 3))', [1, 2]),
            ("safe_double_le", '(c_double is not null and (c_double + 2 <= 3.3))', [1]),
        ]
        overflow_probes = [
            ("overflow_false_positive", 'id in [1, 2] and (c_int + 33 <= 19974)', [2]),
            ("overflow_false_negative", 'id in [1, 2] and (c_int + 1 > 0)', [1, 2]),
        ]
