"""
Minimal validation for the Qdrant FormulaQuery `abs` expression.

This script validates a conservative research subset:
1. Absolute value returns non-negative magnitudes in the tested subset.
2. `abs(0)` returns 0 in the tested subset.
3. Formula defaults can supply a missing input in the tested subset.
4. Missing inputs without defaults are rejected.
5. A single overflowing absolute-value candidate rejects the whole tested query.
...
def build_safe_points() -> list[PointStruct]:
    return [
        PointStruct(id=1, vector=[1.0, 0.0], payload={"value": 2.0, "neg": -3.0, "zero": 0.0}),
        PointStruct(id=2, vector=[0.9, 0.1], payload={"value": -5.0, "neg": -7.0, "zero": 0.0}),
    ]


def build_invalid_points() -> list[PointStruct]:
    return [
        PointStruct(id=10, vector=[1.0, 0.0], payload={"huge_neg": -4e38, "mixed": -4e38}),
        PointStruct(id=11, vector=[0.9, 0.1], payload={"huge_neg": -1.0, "mixed": -1.0}),
    ]
...
def run_transport(prefer_grpc: bool) -> bool:
    ...
    safe_collection = unique_collection_name(
        "abs_formula_safe_grpc" if prefer_grpc else "abs_formula_safe_rest"
    )
    invalid_collection = unique_collection_name(
        "abs_formula_invalid_grpc" if prefer_grpc else "abs_formula_invalid_rest"
    )
    ...
    success_cases = [
        (
            "abs_value",
            FormulaQuery(formula=AbsExpression(abs="value")),
            {1: 2.0, 2: 5.0},
            "Absolute value returns non-negative magnitudes in the tested subset.",
        ),
        (
            "abs_negative_value",
            FormulaQuery(formula=AbsExpression(abs="neg")),
            {1: 3.0, 2: 7.0},
            "Negative inputs are reflected to positive magnitudes in the tested subset.",
        ),
        (
            "abs_zero",
            FormulaQuery(formula=AbsExpression(abs="zero")),
            {1: 0.0, 2: 0.0},
            "The tested `abs(0)` case evaluates to 0 on all validated backends.",
        ),
        (
            "missing_input_with_default",
            FormulaQuery(formula=AbsExpression(abs="missing_input"), defaults={"missing_input": -2.0}),
            {1: 2.0, 2: 2.0},
            "Formula defaults can supply a missing absolute-value input in the tested subset.",
        ),
    ]
    ...
    error_cases = [
        (
            ...
            FormulaQuery(formula=AbsExpression(abs="missing_input")),
            ("missing_input", "No value found"),
            "A missing absolute-value input without defaults is rejected when it must be evaluated.",
        ),
        (
            invalid_collection,
            "overflow_abs_aborts_query",
            FormulaQuery(formula=AbsExpression(abs="huge_neg")),
            ("non-finite", "as f32 = inf"),
            "A single overflowing absolute-value result rejects the whole tested query instead of returning only the finite rows.",
        ),
        (
            invalid_collection,
            "mixed_query_aborts_on_single_overflow_candidate",
            FormulaQuery(formula=AbsExpression(abs="mixed")),
            ("non-finite", "as f32 = inf"),
    ...
    try:
        ...
        print(f"\n--- abs formula validation ({transport}) ---")
...
def run_memory_probe() -> bool:
    safe_collection = unique_collection_name("abs_formula_safe_memory")
    invalid_collection = unique_collection_name("abs_formula_invalid_memory")
...
def main() -> int:
    ...
    print("Qdrant FormulaQuery abs validation")
