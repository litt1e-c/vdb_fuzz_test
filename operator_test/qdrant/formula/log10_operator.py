"""
Minimal validation for the Qdrant FormulaQuery `log10` expression.

This script validates a conservative research subset:
1. Ordinary base-10 logarithms rescore points as expected.
2. `log10(1)` returns 0 in the tested subset.
...
def build_safe_points() -> list[PointStruct]:
    return [
        PointStruct(id=1, vector=[1.0, 0.0], payload={"pos": 100.0, "one": 1.0, "frac": 0.1}),
        PointStruct(id=2, vector=[0.9, 0.1], payload={"pos": 10.0, "one": 1.0, "frac": 0.01}),
    ]


def build_invalid_points() -> list[PointStruct]:
    return [
        PointStruct(id=10, vector=[1.0, 0.0], payload={"mixed": 10.0, "zero": 0.0, "neg": -1.0}),
...
def run_transport(prefer_grpc: bool) -> bool:
    ...
    safe_collection = unique_collection_name(
        "log10_formula_safe_grpc" if prefer_grpc else "log10_formula_safe_rest"
    )
    invalid_collection = unique_collection_name(
        "log10_formula_invalid_grpc" if prefer_grpc else "log10_formula_invalid_rest"
    )
    ...
    success_cases = [
        (
            "safe_log10_positive",
            FormulaQuery(formula=Log10Expression(log10="pos")),
            {1: 2.0, 2: 1.0},
            "Finite positive inputs greater than 1 are rescored as ordinary base-10 logarithms in the tested subset.",
        ),
        (
            "log10_one",
            FormulaQuery(formula=Log10Expression(log10="one")),
            {1: 0.0, 2: 0.0},
            "The tested `log10(1)` case evaluates to 0 on all validated backends.",
        ),
        (
            "log10_fractional_positive",
            FormulaQuery(formula=Log10Expression(log10="frac")),
            {1: math.log10(0.1), 2: math.log10(0.01)},
            "Positive inputs smaller than 1 produce finite negative scores in the tested subset.",
        ),
        (
            "missing_input_with_default",
            FormulaQuery(
                formula=Log10Expression(log10="missing_input"),
                defaults={"missing_input": 10.0},
            ),
            {1: 1.0, 2: 1.0},
    ...
    error_cases = [
        (
            invalid_collection,
            "log10_zero",
            FormulaQuery(formula=Log10Expression(log10="zero")),
            ("non-finite", "log10(0"),
        ...
        (
            invalid_collection,
            "log10_negative",
            FormulaQuery(formula=Log10Expression(log10="neg")),
            ("non-finite", "log10(-"),
        ...
        (
            ...
            FormulaQuery(formula=Log10Expression(log10="missing_input")),
        ...
        (
            ...
            FormulaQuery(formula=Log10Expression(log10="mixed")),
            ("non-finite", "log10(0"),
    ...
    try:
        ...
        print(f"\n--- log10 formula validation ({transport}) ---")
...
def run_memory_probe() -> bool:
    safe_collection = unique_collection_name("log10_formula_safe_memory")
    invalid_collection = unique_collection_name("log10_formula_invalid_memory")
    ...
    try:
        ...
        print("\n--- log10 formula validation (memory) ---")
        print(f"safe_collection={safe_collection}")
        print(f"invalid_collection={invalid_collection}")
...
def main() -> int:
    ...
    print("Qdrant FormulaQuery log10 validation")
